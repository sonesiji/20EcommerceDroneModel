...
class RentalProduct(models.Model):
    ...
    def __str__(self):
        return f"Rental - {self.product.name}"
...
class RentalBooking(models.Model):
    ...
    def clean(self):
        ...
        overlapping_bookings = RentalBooking.objects.filter(
            rental_product=self.rental_product,
    ...
    def calculate_total_cost(self):
        ...
        if days >= 1:
            return self.rental_product.daily_rate * math.ceil(days)
        else:
            return self.rental_product.hourly_rate * math.ceil(hours)
    ...
    def __str__(self):
        return f"Rental Booking - {self.rental_product.product.name} by {self.customer.customer_name}"