...
def slot_booking(request):
    if request.method == 'POST':
        ...
        if existing_booking:
            return HttpResponse('<p style="color: #D8000C; font-size: 14px; font-weight: 600; background: #FFD2D2; padding: 12px; border-left: 5px solid #D8000C; border-radius: 4px; text-align: center;">You can only book one slot per day.</p>', status=400)
...
def payment_page(request, booking_id):
    ...
    if request.method == 'POST':
        ...
        ''', content_type="text/html")  # Ensure the response is HTML
...
from .models import RentalProduct, RentalBooking
from .forms import RentalBookingForm  # Create this form as shown below

def rental_product_list(request):
    rental_products = RentalProduct.objects.filter(is_available=True)
    return render(request, 'rental_product_list.html', {'rental_products': rental_products})

def rental_booking(request, product_id):
    rental_product = get_object_or_404(RentalProduct, id=product_id)
    
    if request.method == 'POST':
        form = RentalBookingForm(request.POST)
        if form.is_valid():
            booking = form.save(commit=False)
            booking.rental_product = rental_product
            booking.customer = request.user.customer  # Assuming user is logged in
            booking.save()
            return redirect('booking_success')  # Redirect to a success page
    else:
        form = RentalBookingForm()

    return render(request, 'rental_booking.html', {'form': form, 'rental_product': rental_product})


def ar_demo_view(request):
    return render(request, 'ar_demo.html')